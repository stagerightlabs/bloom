...
class IPv6 implements XdrTypedef
{
    /**
     * Properties
     */
    use NoConstructor;
    use NoChanges;
    public const MAX_BYTE_LENGTH = 16;
    public string $value;
    ...
    public static function of(string $address): static
    {
        if (!$raw = inet_pton($address)) {
            throw new InvalidArgumentException("The value '{$address}'is not a valid IPv6 address");
        }
    ...
    {
        if (!isset($this->value)) {
            throw new InvalidArgumentException('The IPv6 instance is missing an address');
        }
    ...
    {
        $ipv6 = new static();
        $ipv6->value = $xdr->read(XDR::OPAQUE_FIXED, length: self::MAX_BYTE_LENGTH);
    ...
    public function getAddress(): string
    {
        if (!$address = inet_ntop($this->value)) {
            throw new UnexpectedValueException('The IPv6 instance contains an invalid address');
        }
    ...
    public function withRaw(string $value): static
    {
        ...
        if ($length > $max) {
            throw new InvalidArgumentException("Attempting to use a {$length} byte string for an IPv6 address where only {$max} bytes are allowed");
        }
