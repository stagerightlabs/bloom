...
final class TransactionV0Envelope implements Envelope, XdrStruct
{
    /**
     * Properties
     */
    protected TransactionV0 $transaction;
    ...
    public function toXdr(XDR &$xdr): void
    {
        if (!isset($this->transaction)) {
            throw new InvalidArgumentException('The transaction envelope is missing a transaction');
        }

        if (!isset($this->signatures)) {
            throw new InvalidArgumentException('The transaction envelope is missing a signature list');
        }
    ...
    {
        $envelope = new static();
        $envelope->transaction = $xdr->read(TransactionV0::class);
     ...
     * Set the transaction.
     *
     * @param TransactionV0 $transaction
     * @return static
     */
    public function withTransaction(TransactionV0 $transaction): static
    {
    ...
    public function getSignaturePayload(string $network): TransactionSignaturePayload
    {
        return TransactionSignaturePayload::for(
            TransactionSignaturePayloadTaggedTransaction::wrapTransaction($this->transaction),
            $network
        );
    }
